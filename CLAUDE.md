...
Las tres usan `'use cache'` + `cacheLife('hours')` + `cacheTag('posts')`. Las de guías (`getAllGuides`/`getGuideBySlug`) usan `cacheTag('guides')`. El `cacheLife('hours')` es el tope de frescura (revalida sola cada ~1h), pero además hay **revalidación on-demand**: el webhook de Contentful llama a `revalidateTag('posts'|'guides')` al publicar, así los cambios (post nuevo, precio de guía) se ven **en segundos** sin esperar la hora ni redeployar. El mapeo Contentful→`BlogPost` está en `mapEntry`: lee el campo `coverImage` y le antepone `https:` a la URL del asset.
...
- [x] **Envío al publicar (cableado)** ✅ Webhook de Contentful (Entry→Publish) → `app/api/webhooks/contentful/route.ts`: verifica header `x-webhook-secret` (`CONTENTFUL_WEBHOOK_SECRET`) y **ramifica por `sys.contentType.sys.id`**: si es `guia` → `revalidateTag('guides')` y responde; si es `blogPost` → `revalidateTag('posts')` + obtiene el post (`getPostBySlug`) y hace `resend.broadcasts.create({ segmentId, from, subject, html, send:true })`. Responde siempre 200 (salvo 401) para no reintentar. ⚠️ **El webhook debe estar suscrito a los publish de AMBOS content types** (`blogPost` y `guia`) para que la revalidación de precios funcione.
